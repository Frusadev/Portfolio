  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-background border-4 border-red-950 p-6 shadow-[8px_8px_0px_0px_rgba(69,10,10,1)]">
        <h2 className="text-xl font-bold uppercase tracking-widest text-red-950 mb-4">
          Create New Series
        </h2>
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-xs uppercase font-bold text-red-950 tracking-wider">
                Title
              </label>
              <input
                value={title}
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {series.map((item) => (
              <tr
                key={item.id}
                className="border-b-2 border-red-950/20 hover:bg-red-950/5 transition-colors"
              >
                <td className="p-4 font-bold text-red-950">{item.title}</td>
                <td className="p-4 text-red-950/80 font-mono text-sm">
                  {item.slug}
                </td>
            ...
            {series.length === 0 && (
              <tr>
                <td
                  colSpan={5}
                  className="p-8 text-center text-red-950/60 font-bold uppercase tracking-widest"
                >
                  No series found
                </td>
              </tr>
