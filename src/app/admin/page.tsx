  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center pb-6 border-b-4 border-red-950">
        <h2 className="text-3xl font-bold uppercase tracking-wider text-red-950">
          Dashboard
        </h2>
      </div>
      ...
      <div>
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <div
              key={item.id}
              className="p-4 border-b-2 border-red-950 last:border-0 hover:bg-red-950/5 flex justify-between items-center group"
            >
              <span className="font-bold truncate max-w-[70%]">
                {type === "post" ? item.title : item.name || item.title}
              </span>
